...
| --- | --- | --- |
| `"scalar"` (default) | `ScalarDelta<T>` | `ScalarDelta::Changed(new)` when the values differ. |
...
assert_eq!(delta.online, ScalarDelta::Changed(true));
```
...
`#[serde(default, skip_serializing_if = "::delta_struct::ScalarDelta::is_unchanged")]`
through a field-level `delta_leader` keeps unchanged fields out of the payload entirely, rather than
sending them as `"unchanged"`.
